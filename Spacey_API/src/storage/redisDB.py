...
class redis_database(object):
    ...
    def register_user(self, name, key):
        ...
        if self.client.sismember("registered_users", name):
            return False
    ...
    def get_registered_restaurants(self):
        if len(self.res_list):
            self.res_list.clear()
        for i in range(
            0, self.client.llen("{}_registered_restaurants".format(self.user))
        ):
            self.res_list.append(
                self.client.lindex("{}_registered_restaurants".format(self.user), i)
            )
    ...
    def clearUser(self, user):
        while self.client.llen("{}_registered_restaurants".format(user)):
            self.clearDB(
                self.user
                + "_"
                + self.client.lpop("{}_registered_restaurants".format(user))
            )
    ...
    def importFromDB(self, session_name, export_to_script=None, mode="client"):
        json_list = self.configJsonDir()
        export_limit = 0
        if export_to_script is not None:
            export_limit = len(export_to_script)
        for i in json_list:
            data = {}
            name = session_name + "_" + self.getbasename(i)
            print(name)
