...
class myCanvasObject(object):
    def __init__(self,frame, width, height):
        self.canvas = Canvas(frame, width = width, height = height, highlightcolor = tkinter_window_cfg.hlcolor, highlightthickness = 5, bg="SteelBlue1")
        self.line_obj = []
        self.rec_obj = {}
        self.border_obj = []
    ...
    def resetCursorLocation(self):
        self.canvas.move(tkinter_window_cfg.cursor, tkinter_window_cfg.x_list[0] - tkinter_window_cfg.x, tkinter_window_cfg.y_list[0] - tkinter_window_cfg.y)
        tkinter_window_cfg.x = tkinter_window_cfg.x_list[0]
        tkinter_window_cfg.y = tkinter_window_cfg.y_list[0]
        
        self.canvas.move(tkinter_window_cfg.cursor, tkinter_window_cfg.x_list[int(tkinter_window_cfg.scale/2)] - tkinter_window_cfg.x, tkinter_window_cfg.y_list[int(tkinter_window_cfg.scale/2)] - tkinter_window_cfg.y)
        tkinter_window_cfg.x = tkinter_window_cfg.x_list[int(tkinter_window_cfg.scale/2)]
        tkinter_window_cfg.y = tkinter_window_cfg.y_list[int(tkinter_window_cfg.scale/2)]
    ...
    def deleteNode(self, idx):
        if tkinter_window_cfg.prev_node_idx is idx: tkinter_window_cfg.prev_node_idx = None
        print("canvas")
    ...
    def restoreTagOrder(self):
        ...
        if tkinter_window_cfg.image_flag: self.canvas.tag_raise(self.floorplan_obj)
        for i in self.rec_obj.values():
            self.canvas.tag_raise(i)
    ...
    def placeNode(self, idx, x, y):
        self.xlen, self.ylen = tkinter_window_cfg.box_len, tkinter_window_cfg.box_len #size of node
...
class CanvasGridFrame(object):
    def __init__(self,canvas, num):
        self.xpos, self.ypos, self.xlen, self.ylen = 0, 0, tkinter_window_cfg.canvas_w, tkinter_window_cfg.canvas_h
        self.canvas = canvas
        self.deviceID = 0
        tkinter_window_cfg.bg = canvas.create_rectangle(self.xpos, self.ypos, self.xpos + self.xlen, self.ypos+self.ylen, fill="SteelBlue1", width = 0)
    ...
    def createGrid(self):
        ...
        for i in range(num+10):
            tkinter_window_cfg.x_list.append(round(self.xpos+i*tkinter_window_cfg.step))
            tkinter_window_cfg.y_list.append(round(self.ypos+i*tkinter_window_cfg.step))
            tkinter_window_cfg.myCanvas.line_obj.append(self.canvas.create_line(self.xpos+i*tkinter_window_cfg.step, self.ypos, self.xpos+i*tkinter_window_cfg.step, self.ypos + self.ylen, fill = "sky blue", width = 2))
            tkinter_window_cfg.myCanvas.line_obj.append(self.canvas.create_line(self.xpos, self.ypos+i*tkinter_window_cfg.step, self.xpos+self.xlen, self.ypos+i*tkinter_window_cfg.step, fill = "sky blue", width = 2))

 
        tkinter_window_cfg.num_coordinates_max = (tkinter_window_cfg.canvas_w/tkinter_window_cfg.step-1) * (tkinter_window_cfg.canvas_h/tkinter_window_cfg.step-1)
    ...
    def drawBoundary(self, step):
        if int(tkinter_window_cfg.canvas_w/step) > len(tkinter_window_cfg.x_list): return
        print("step: "+str(step))
        ...
        tkinter_window_cfg.x_bb1, tkinter_window_cfg.y_bb1, tkinter_window_cfg.x_bb2, tkinter_window_cfg.y_bb2 = x2,y2,_x2, _y2

        #if no image/ image deleted, set future image to fit the canvas as much as possible
        if tkinter_window_cfg.img_x_bb1 is -1:  tkinter_window_cfg.img_x_bb1, tkinter_window_cfg.img_y_bb1 = tkinter_window_cfg.x_bb1, tkinter_window_cfg.y_bb1
        ...
        print("_y2: " + str(_y2))
        """
        tkinter_window_cfg.myCanvas.border_obj.append(self.canvas.create_rectangle(x1,y1,_x1+step*2,y2, fill = "gray10"))    #north    
        tkinter_window_cfg.myCanvas.border_obj.append(self.canvas.create_rectangle(x1,_y2,_x1+step*2,_y1+step*10, fill = "gray10")) #south
        tkinter_window_cfg.myCanvas.border_obj.append(self.canvas.create_rectangle(x1,y2,x2,_y2, fill = "gray10")) #west
        tkinter_window_cfg.myCanvas.border_obj.append(self.canvas.create_rectangle(_x2,y2,_x1+step*2,_y2, fill = "gray10")) #east


    def refresh(self, delete = True, resize = True):
        tkinter_window_cfg.load_flag = not resize
        if delete: tkinter_window_cfg.res.deleteAllNodes()
        tkinter_window_cfg.myCanvas.deleteAllGrids()
        self.grid = self.createGrid()
        # If need resize and there is an image to resize
        if resize and tkinter_window_cfg.image_flag: tkinter_window_cfg.img.resize()
...
class CursorNode(object):
    def __init__(self, canvas):
        ...
        tkinter_window_cfg.x, tkinter_window_cfg.y = int(self.xpos + self.xlen/2), int(self.ypos + self.ylen/2)
        tkinter_window_cfg.cursor = canvas.create_rectangle(self.xpos, self.ypos,self.xpos+self.xlen, self.ypos+self.ylen, fill = "red")
        ...
        self.restaurant = tkinter_window_cfg.res
        self.move_flag = False
        self.callBack = []
    
    def deleteNode(self, event):
        if (tkinter_window_cfg.x, tkinter_window_cfg.y) in self.restaurant.dict_sensor_motes:
            print("deleted")            
            self.restaurant.deleteNode(tkinter_window_cfg.x, tkinter_window_cfg.y)
            tkinter_window_cfg.error.updateText("Node deleted at x:{x} and y:{y}".format(x = tkinter_window_cfg.x, y = tkinter_window_cfg.y), "deep pink")
    ...
    def move(self, event):
        if self.move_flag:
            ...
            self.canvas.move(tkinter_window_cfg.cursor, new_xpos-self.mouse_xpos ,new_ypos-self.mouse_ypos)
    ...
    def release(self, event):
        new_xpos, new_ypos = self.estimate(event.x,tkinter_window_cfg.x_list), self.estimate(event.y,tkinter_window_cfg.y_list)
        self.canvas.move(tkinter_window_cfg.cursor, new_xpos-tkinter_window_cfg.x ,new_ypos-tkinter_window_cfg.y)
        self.mouse_xpos = new_xpos
        self.mouse_ypos = new_ypos
        self.move_flag = False
    ...
    def nodeDetectCallback(self):
        ...
        if (tkinter_window_cfg.x, tkinter_window_cfg.y) in self.restaurant.dict_sensor_motes:
            text = self.restaurant.printMoteAt(tkinter_window_cfg.x, tkinter_window_cfg.y)
            tkinter_window_cfg.node_idx = self.restaurant.dict_sensor_motes.get((tkinter_window_cfg.x, tkinter_window_cfg.y)).idx
            tkinter_window_cfg.myCanvas.canvas.itemconfig(tkinter_window_cfg.myCanvas.rec_obj[tkinter_window_cfg.node_idx], fill = "orange")
            if tkinter_window_cfg.prev_node_idx is not None and tkinter_window_cfg.prev_node_idx is not tkinter_window_cfg.node_idx: tkinter_window_cfg.myCanvas.canvas.itemconfig(tkinter_window_cfg.myCanvas.rec_obj[tkinter_window_cfg.prev_node_idx], fill = "blue")
            ...
            if tkinter_window_cfg.prev_node_idx is not None: tkinter_window_cfg.myCanvas.canvas.itemconfig(tkinter_window_cfg.myCanvas.rec_obj[tkinter_window_cfg.prev_node_idx], fill = "blue")
            tkinter_window_cfg.deposit_flag = False
            self.callBack[0]("No node information") # Print text on status label
            if tkinter_window_cfg.err_inval_input: self.callBack[1]("MediumPurple1")
...
class menu_upload(object):
    def __init__(self, frame, width, height):
        ...
        self.labelFrame.pack(fill = X, side = TOP, pady = tkinter_window_cfg.pady, padx = tkinter_window_cfg.padx)
        self.obj = Button(self.labelFrame, text = "Import Floor plan", command = self.fileupload, bg = tkinter_window_cfg.buttcolor)
        self.obj.pack(ipadx = 10, ipady = 10, fill = X, side = TOP)
        self.obj = Button(self.labelFrame, text = "Clear Floor plan", command = self.floorplanclear, bg = tkinter_window_cfg.buttcolor)
        self.obj.pack(ipadx = 10, ipady = 10, fill = X, side = TOP)

    def fileupload(self):
        tkinter_window_cfg.myCanvas.deleteImage()
        tkinter_window_cfg.res.deleteAllNodes()
        try:
            filename = filedialog.askopenfilename(initialdir = tkinter_window_cfg.floorplan_folder_input, title = "Select File", filetypes = (("all files","*.*"),("png files","*.png"), ("jpeg files","*.jpg")))
        except:
            tkinter_window_cfg.error.updateText("Upload failed", "red")
            return
        # Create Image generator
        tkinter_window_cfg.img = imgpro.floorPlan(filename, tkinter_window_cfg.myCanvas.canvas)
...
class map_refresh(object):
    def __init__(self, frame,factor, width, height):
        ...
        self.labelFrame.pack(fill = X, side = TOP, pady = tkinter_window_cfg.pady, padx = tkinter_window_cfg.padx)
        self.but1 = Button(self.labelFrame, text = "Scale up", command = self.updateUp, bg = tkinter_window_cfg.buttcolor)
        self.but1.pack(ipadx = 10, ipady = 10, fill = X)
        self.but2 = Button(self.labelFrame, text = "Scale down", command = self.updateDown, bg = tkinter_window_cfg.buttcolor)
    ...
    def updateUp(self):
        ...
        if tkinter_window_cfg.scale >= 120:
            tkinter_window_cfg.error.updateText("[Grid] Cannot scale up any further", "orange")
            return
    ...
    def updateDown(self):
        ...
        if (tkinter_window_cfg.scale - self.factor) <= 0 or int(tkinter_window_cfg.canvas_w/(tkinter_window_cfg.scale - self.factor)) > tkinter_window_cfg.max_step:
            tkinter_window_cfg.error.updateText("[Grid] Cannot scale down any further", "orange")
            return
...
class menu_devinfo(object):
    ...
    def setEntryRow(self,i):
        ...
        self.entryList.append(Entry(self.rowFrame[i], bd = 5, textvariable = self.text[i], highlightcolor = tkinter_window_cfg.hlcolor, highlightthickness = 5, highlightbackground = "gray55", width = 6))
    ...
    def enter(self, event, id):
        ...
        if numErrors: 
            ...
            tkinter_window_cfg.error.updateText(self.error_text[self.needCorrect.index(True)][0], self.error_text[self.needCorrect.index(True)][1])
        ...
        if tkinter_window_cfg.res.registerNode(tkinter_window_cfg.x, tkinter_window_cfg.y, x[0], x[1], x[2], tkinter_window_cfg.node) is False:
            tkinter_window_cfg.err_inval_input = True
            self.highlightDeviceInfo("red")
            tkinter_window_cfg.error.updateText("[KeyError] Repeated cluster number! Key in a different combination of <cluster ID> <cluster level> <sensor ID>", "red")
        ...
        tkinter_window_cfg.res.printMoteAt(tkinter_window_cfg.x, tkinter_window_cfg.y)
...
class menu_status(object):
    def __init__(self, frame, width, height):
        ...
        self.labelFrame.pack(fill = X, padx = tkinter_window_cfg.padx, pady = tkinter_window_cfg.pady, side = TOP)
...
class menu_help(object):
    ...
    def helpMessage(self, frame):
        ...
        text = Text(frame,  bg = "white", font = tkinter_window_cfg.help_font, bd = 10, wrap = WORD)
...
class menu_debug(object):
    def __init__(self, frame, width, height):
        ...
        tkinter_window_cfg.error_font = font.Font(family="Courier New", size=8, weight = "bold")
        self.frame = frame
        self.labelFrame = LabelFrame(self.frame, text = "Debugger", bg = "gray55", height = 500, width = 550)
        self.labelFrame.pack(fill = X, padx = tkinter_window_cfg.padx, pady = tkinter_window_cfg.pady, side = TOP)
        self.labelFrame.pack_propagate(0)
        self.obj = Listbox(self.labelFrame,height = 300, width = 550, bg = "gray10", font = tkinter_window_cfg.error_font)
...
class img_xyshift(object):
    def __init__(self, frame, factor, width, height):
        ...
        self.but1 = Button(self.framex, text = "Left", command = self.left, width = 2, bg = tkinter_window_cfg.buttcolor)
        self.but1.pack(ipadx = 10, ipady = 10, side = LEFT)
        self.but2 = Button(self.framex, text = "Right", command = self.right, width = 2, bg = tkinter_window_cfg.buttcolor)
        self.but2.pack(ipadx = 10, ipady = 10, side = LEFT)

        self.but1 = Button(self.framey, text = "Up", command = self.up, width = 2, bg = tkinter_window_cfg.buttcolor)
        self.but1.pack(ipadx = 10, ipady = 10, side = LEFT)
        self.but2 = Button(self.framey, text = "Down", command = self.down, width = 2, bg = tkinter_window_cfg.buttcolor)
        self.but2.pack(ipadx = 10, ipady = 10, side = LEFT)
        ...
        self.but1 = Button(self.framex2, text = "Expand", command = self.s_up, width = 8, bg = tkinter_window_cfg.buttcolor)
        self.but1.pack(ipadx = 10, ipady = 10, fill = X, side = TOP)
        self.but2 = Button(self.framey2, text = "Contract", command = self.s_down, width = 8, bg = tkinter_window_cfg.buttcolor)
    ...
    def left(self):
        ...
        tkinter_window_cfg.myCanvas.canvas.move(tkinter_window_cfg.myCanvas.floorplan_obj, -self.factor, 0)
    def right(self):
        tkinter_window_cfg.img_x_bb1 += self.factor 
        tkinter_window_cfg.myCanvas.canvas.move(tkinter_window_cfg.myCanvas.floorplan_obj, self.factor, 0)
    ...
    def up(self):
        ...
        tkinter_window_cfg.myCanvas.canvas.move(tkinter_window_cfg.myCanvas.floorplan_obj, 0, -self.factor)
    def down(self):
        tkinter_window_cfg.img_y_bb1 += self.factor 
        tkinter_window_cfg.myCanvas.canvas.move(tkinter_window_cfg.myCanvas.floorplan_obj, 0, self.factor)
    ...
    def s_up(self):
        if tkinter_window_cfg.myCanvas.floorplan_obj is None: 
            tkinter_window_cfg.error.updateText("<Floor Plan> Cannot scale down before inserting floor plan!", "orange")
            return
        if tkinter_window_cfg.img_padding == 0: 
            tkinter_window_cfg.error.updateText("<Floor Plan> Cannot scale down further", "orange")
            return
...
class img_scaleshift(object):
    def __init__(self, frame, factor, width, height):
        ...
        self.labelFrame.pack(fill = X, side = TOP, pady = tkinter_window_cfg.pady, padx = tkinter_window_cfg.padx)
        self.but1 = Button(self.labelFrame, text = "Size +", command = self.up, width = 4, bg = tkinter_window_cfg.buttcolor)
        self.but1.pack(ipadx = 10, ipady = 10, side = LEFT)
        self.but2 = Button(self.labelFrame, text = "Size -", command = self.down, width = 4, bg = tkinter_window_cfg.buttcolor)
        self.but2.pack(ipadx = 10, ipady = 10, side = LEFT)
        self.factor = factor
...
class node_scaleshift(object):
    def __init__(self, frame, factor, width, height):
        ...
        self.labelFrame.pack(fill = X, side = TOP, pady = tkinter_window_cfg.pady, padx = tkinter_window_cfg.padx)
        self.but1 = Button(self.labelFrame, text = "Up", command = self.up, width = 4, bg = tkinter_window_cfg.buttcolor)
        self.but1.pack(ipadx = 10, ipady = 10, side = LEFT, padx = 25)
        self.but2 = Button(self.labelFrame, text = "Down", command = self.down, width = 4, bg = tkinter_window_cfg.buttcolor)
        self.but2.pack(ipadx = 10, ipady = 10, side = LEFT, padx = 12)
        self.factor = factor

    def up(self):
        tkinter_window_cfg.box_len += self.factor
        tkinter_window_cfg.error.updateText("Node length: {n}".format(n = tkinter_window_cfg.box_len), "yellow")
        tkinter_window_cfg.res.changeNodeSize()
    def down(self):
        tkinter_window_cfg.box_len -= self.factor
        tkinter_window_cfg.res.changeNodeSize()
...
class json_viewer(object):
    def __init__(self, frame, width, height):
        #cfg.error_font = font.Font(family = "Times", font = "3", weight =  "BOLD")
        tkinter_window_cfg.json_font = font.Font(family="Courier New", size=8, weight = "bold")
        self.frame = frame
        ...
        self.butt1 = Button(self.frame2, text = "Import JSON [Local]", command = self.download, width = 550, bg = tkinter_window_cfg.buttcolor)
        self.butt1.pack()
        self.butt = Button(self.frame2, text = "Export JSON [Local]", command = self.upload, width = 550, bg = tkinter_window_cfg.buttcolor)
        self.butt.pack()
        ...
        self.optionmenu = OptionMenu(self.dbframe, self.dbselect, *tkinter_window_cfg.db_options)
        ...
        self.butt3 = Button(self.dbframe, text = "Export to DB", command = partial(self.displayUploadMenu, tkinter_window_cfg.root), width = 550, bg = "RosyBrown1")
        ...
        self.buttupdate = Button(self.dbframe, text = "Update Restaurant Information", command = partial(self.displayUploadMenu, tkinter_window_cfg.root, True), width = 550, bg = "sky blue")
        self.buttupdate.pack_forget()
    
        #self.butt.pack(ipadx = 30, ipady = 30, fill = X)
        self.obj = Text(self.frame1,  width = 550, height = 190, bg = "gray10", font = tkinter_window_cfg.json_font)
    ...
    def refreshDB(self):
        if not internet_on():
            tkinter_window_cfg.error.updateText("[DB] No Network... cannot connect to database", "red")
            return

        tkinter_window_cfg.error.updateText("[DB] Connecting...", "yellow")
        
        err = tkinter_window_cfg.database.timeout()
        if err == 1: tkinter_window_cfg.error.updateText("[DB] No Network... cannot connect to database", "red")
        else:

            tkinter_window_cfg.error.updateText("[DB] Connected to database", "pale green")
    ...
    def DBclearUser(self):
        ...
        tkinter_window_cfg.error.updateText("Acc Del: {}. See you!".format(tkinter_window_cfg.userid), "yellow")
    ...
    def DBclearDB(self):
        ...
        tkinter_window_cfg.error.updateText("Restaurant deleted: {}".format(name), "yellow")
        self.refresh()
    ...
    def refresh(self):
        ...
        new_choices = ["Enter New Restaurant"] + tkinter_window_cfg.database.get_registered_restaurants()
    ...
    def callback(self, *args):
        if self.dbselect.get() == "No Database Selected":
            ...
            tkinter_window_cfg.res_info = tkinter_window_cfg.database.getResInfo(tkinter_window_cfg.userid+"_"+tkinter_window_cfg.session_name)
    ...
    def downloadDB(self): 
        while True: 
            if tkinter_window_cfg.error.updateText("Downloading, please wait...", "yellow") == True: break
        
        if self.dbselect.get() in ["No Database Selected", "No restaurants stored"]: return
        tkinter_window_cfg.local_disk = False
        tkinter_window_cfg.session_name = self.dbselect.get()
        if tkinter_window_cfg.res.size: tkinter_window_cfg.res.deleteAllNodes()
        if tkinter_window_cfg.image_flag: tkinter_window_cfg.myCanvas.deleteImage()
        str1 = tkinter_window_cfg.decompile(tkinter_window_cfg.json_folder, local_disk  = False)
    ...
    def upload(self):
        ...
        time.sleep(2)
        if tkinter_window_cfg.res.size == 0:
            tkinter_window_cfg.error.updateText("[ERR] Need at least 1 node", "red")
            return
        tkinter_window_cfg.local_disk = True
        try:
            tkinter_window_cfg.json_path = filedialog.asksaveasfilename(initialdir = tkinter_window_cfg.json_folder_config, title = "Select File", filetypes = [("Json File", "*.json")])
        except:
            tkinter_window_cfg.error.updateText("Upload failed", "red")
        tkinter_window_cfg.session_name = tkinter_window_cfg.getbasename(tkinter_window_cfg.json_path)
        
        if tkinter_window_cfg.img is not None: tkinter_window_cfg.img.save()
    ...
    def displayUploadMenu(self, root, update = False):
        ...
        if tkinter_window_cfg.res.size == 0 and update == False:
            tkinter_window_cfg.error.updateText("[ERR] Need at least 1 node", "red")
            return
        ...
        if update: 
            ...
            for i in tkinter_window_cfg.res_info_op:
                ...
                else                                  : temp = tkinter_window_cfg.getvar()[i].rstrip('\n')
                if i == "res_addr":
    ...
    def updateInfo(self, push_floor_plan):
        ...
        if push_floor_plan: 
            ...
            if tkinter_window_cfg.database.setResInfo(tkinter_window_cfg.userid+"_"+tkinter_window_cfg.session_name, tkinter_window_cfg.resinfo): 
                tkinter_window_cfg.error.updateText("Updated Restaurant info!", "pale green")
    ...
    def login(self): 
        ...
        if err == 0: 
            ...
            tkinter_window_cfg.error.updateText("[DB] Welcome: {}".format(tkinter_window_cfg.userid), "pale green")
            self.helpMenu.destroy()
            self.refresh()
        else:
            tkinter_window_cfg.error.updateText("[DB] Connection Timed out, try again", "red")
    ...
    def register(self): 
        ...
        if err == 0:
            ...
            tkinter_window_cfg.userid = name
            tkinter_window_cfg.error.updateText("[DB] Account created: {}".format(tkinter_window_cfg.userid), "pale green")
            self.helpMenu.destroy()
            self.refresh()
        else:
            tkinter_window_cfg.error.updateText("[DB] Connection Timed out, try again", "red")
    ...
    def uploadDB(self, event = None):
        while True: 
            if tkinter_window_cfg.error.updateText("Uploading, please wait...", "yellow") == True: break
        if self.session_name_set is False:
            ...
            self.helpMenu.destroy()
        self.session_name_set = False
        tkinter_window_cfg.error.updateText("[DB] Created new session: "+ str(tkinter_window_cfg.session_name), "pale green")
        tkinter_window_cfg.local_disk  = False
        ...
        imagegen() # Generates template
        str1 = tkinter_window_cfg.compile(tkinter_window_cfg.json_folder, local_disk = False)
        self.updateText(str1+"\n", "p")
        self.updateText(">>>"+"-"*15+"\n", "b")
        tkinter_window_cfg.error.updateText("JSON Updated successfully", "pale green")
        self.refresh()
    ...
    def download(self):
        tkinter_window_cfg.local_disk = True
        try:
            filename = filedialog.askopenfilename(initialdir = tkinter_window_cfg.json_folder_config, title = "Select File", filetypes = [("Json File", "*.json")])
        except:
