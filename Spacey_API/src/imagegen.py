...
def imagegen():
    ...
    node_off = node_off.resize((int(tkinter_window_cfg.box_len*2.5), tkinter_window_cfg.box_len*2))
    node_on = Image.open(tkinter_window_cfg.nodeOn_path)
    node_on = node_on.resize((int(tkinter_window_cfg.box_len*2.5), tkinter_window_cfg.box_len*2))
    ...
    bg = Image.new('RGBA', (tkinter_window_cfg.canvas_xlen, tkinter_window_cfg.canvas_ylen), (92, 152, 226, 255))
    ...
    for i in tkinter_window_cfg.res.idxList:
        x = tkinter_window_cfg.res.x_coord[i]- (tkinter_window_cfg.x_bb1 + tkinter_window_cfg.box_len)
        y = tkinter_window_cfg.res.y_coord[i] - (tkinter_window_cfg.y_bb1 + tkinter_window_cfg.box_len)
        ...
        if int(tkinter_window_cfg.res.occupancy[i]) == 0: node = node_on
        else: node = node_off
