...
class floorPlan(object):
    ...
    def __init__(self, path, canvas, mode = True):
        ...
        self.resize()
        self.x_mid = int(tkinter_window_cfg.x_bb1 + (tkinter_window_cfg.x_bb2 - tkinter_window_cfg.x_bb1)/2)
        self.y_mid = int(tkinter_window_cfg.y_bb1 + (tkinter_window_cfg.y_bb2 - tkinter_window_cfg.y_bb1)/2)
    ...
    def resize(self):
        ...
        if tkinter_window_cfg.myCanvas.floorplan_obj is not None: self.canvas.delete(tkinter_window_cfg.myCanvas.floorplan_obj)

        self.x_mid = int(tkinter_window_cfg.x_bb1 + (tkinter_window_cfg.x_bb2 - tkinter_window_cfg.x_bb1)/2)
        self.y_mid = int(tkinter_window_cfg.y_bb1 + (tkinter_window_cfg.y_bb2 - tkinter_window_cfg.y_bb1)/2)
        width_r = tkinter_window_cfg.x_bb2 - tkinter_window_cfg.x_bb1 - tkinter_window_cfg.img_padding
        height_r = tkinter_window_cfg.y_bb2 - tkinter_window_cfg.y_bb1 - tkinter_window_cfg.img_padding
        ...
        tkinter_window_cfg.myCanvas.floorplan_obj = self.canvas.create_image(tkinter_window_cfg.img_x_bb1,tkinter_window_cfg.img_y_bb1, anchor = "nw", image = self.photoimg)
